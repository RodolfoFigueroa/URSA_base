...
import ursa_base.degree_of_urbanization as dou
import ursa_base.ghsl as ghsl
...
def derive_auxiliary_roads(roads, d_metric=np.inf):
    ...
    ii, jj = roads.values.shape
    grid_i, grid_j = np.meshgrid(range(ii), range(jj), indexing="ij")
